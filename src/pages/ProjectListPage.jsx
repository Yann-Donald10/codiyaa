...
const ProjectListPage = () => {
  ...
  const [projects, setProjects] = useState([]);
    ...
    const fetchProjects = async () => {
      try {
        const { data, error } = await supabase
          .from("student")
          .select("id_student, student_firstname, student_lastname, student_code, url_project")
    ...
    const handleChangeStatus = async (newStatus) => {
      ...
      // 2) If session is activated → generate codes
      if (newStatus === true) {
  ...
  return (
  <div>
      <Navbarproject session_status = {session_status} handleLogout={handleLogout} handleChangeStatus={handleChangeStatus}/>
      <ProjectList loading = {loading} error={error} projects={projects} fetchProjects={fetchProjects} session_status = {session_status}/>
    </div>
  )
}
